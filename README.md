...
1. Разделение чтения данных и расчётов через абстрактный класс `DataReader`
   позволило добавить обработчик формата варианта, не изменив ни одной строки в
   расчётных классах и в точке входа: новый обработчик достаточно
   зарегистрировать в словаре `READERS` фабрики. Это практическая иллюстрация
   принципа открытости/закрытости.
2. Вынесение параметров варианта (`TARGET_SCORE`, `MIN_SUBJECTS`) в константы
   класса, а не в тела методов, сделало методы пригодными к повторному
   использованию с другими значениями — именно это позволило проверить
   процедуру тестами при разных порогах, а не только на единственном сочетании
   из условия.
3. Ветвление Git отделяет незавершённую работу от главной ветки: обработчик
   YAML и расчётный класс разрабатывались в отдельных ветках и попадали в
   `main` только после того, как все тесты проекта проходили. Слияние ключом
   `--no-ff` сохраняет историю ветки в журнале проекта.
4. Непрерывная интеграция превращает договорённости о качестве кода в
   автоматическую проверку: несоответствие PEP 8 или упавший тест помечают
   сборку красным крестиком сразу после отправки изменений, а не на защите
   работы. Намеренно внесённая в код ошибка позволила увидеть весь цикл —
   красный статус, разбор журнала сборки, исправление и зелёный статус.
